...
describe('memory.js', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'jarvis-memory-'));
    jarvisMdPath = path.join(tmpDir, 'JARVIS.md');
  ...
  describe('remember()', () => {
    ...
    it('creates memory directory if not exists', () => {
      const memDir = path.join(tmpDir, '.jarvis', 'memory');
  ...
  describe('loadProjectInstructions()', () => {
    it('returns empty string when JARVIS.md does not exist', () => {
      expect(memory.loadProjectInstructions()).toBe('');
    });

    it('returns JARVIS.md contents', () => {
      fs.writeFileSync(jarvisMdPath, '# My Project\nUse yarn instead of npm\n', 'utf-8');
      expect(memory.loadProjectInstructions()).toContain('My Project');
    });

    it('trims whitespace', () => {
      fs.writeFileSync(jarvisMdPath, '  hello  \n\n', 'utf-8');
      expect(memory.loadProjectInstructions()).toBe('hello');
    });
  });

  describe('getMemoryContext()', () => {
    it('returns empty string when no memories and no JARVIS.md', () => {
      expect(memory.getMemoryContext()).toBe('');
    });

    it('includes JARVIS.md content', () => {
      fs.writeFileSync(jarvisMdPath, '# Instructions\nBe helpful', 'utf-8');
    ...
    it('includes both JARVIS.md and memories', () => {
      fs.writeFileSync(jarvisMdPath, 'Project rules here', 'utf-8');
