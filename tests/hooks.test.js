...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'jarvis-hooks-'));
...
describe('hooks.js', () => {
  ...
  describe('loadHookConfig()', () => {
    ...
    it('returns empty object when config has no hooks', () => {
      const configDir = path.join(tmpDir, '.jarvis');
  ...
  describe('getHooksForEvent()', () => {
    ...
    it('finds hook script files in .jarvis/hooks/', () => {
      const hooksDir = path.join(tmpDir, '.jarvis', 'hooks');
    ...
    it('combines script files and config hooks', () => {
      ...
      fs.writeFileSync(path.join(hooksDir, 'pre-tool'), '#!/bin/bash\necho script');
      fs.writeFileSync(
        path.join(tmpDir, '.jarvis', 'config.json'),
  ...
  describe('executeHook()', () => {
    ...
    it('passes environment variables', () => {
      const result = executeHook('echo $JARVIS_TEST_VAR', { JARVIS_TEST_VAR: 'works' });
      expect(result.success).toBe(true);
  ...
  describe('runHooks()', () => {
    ...
    it('passes context as JARVIS_ env vars', () => {
      const configDir = path.join(tmpDir, '.jarvis');
      fs.mkdirSync(configDir, { recursive: true });
      fs.writeFileSync(
        path.join(configDir, 'config.json'),
        JSON.stringify({ hooks: { 'post-tool': ['echo $JARVIS_TOOL_NAME'] } })
      );
  ...
  describe('initHooksDir()', () => {
    it('creates .jarvis/hooks/ directory', () => {
      const dir = initHooksDir();
      expect(fs.existsSync(dir)).toBe(true);
      expect(dir).toContain('.jarvis');
