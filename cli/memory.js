 ...
 * Persistent key-value memory stored in .jarvis/memory/
 * Also loads JARVIS.md from project root (like CLAUDE.md)
 */
...
function getMemoryDir() {
  return path.join(process.cwd(), '.jarvis', 'memory');
}
...
function getJarvisMdPath() {
  return path.join(process.cwd(), 'JARVIS.md');
}
...
/**
 * Load JARVIS.md from project root (if it exists)
 * @returns {string} — Contents of JARVIS.md or empty string
 */
function loadProjectInstructions() {
  const jarvisMd = getJarvisMdPath();
  if (!fs.existsSync(jarvisMd)) return '';
  try {
    return fs.readFileSync(jarvisMd, 'utf-8').trim();
  } catch {
    return '';
  }
}
 ...
 * Returns formatted string with memories + JARVIS.md content
 * @returns {string}
 */
function getMemoryContext() {
  ...
  if (instructions) {
    parts.push(`PROJECT INSTRUCTIONS (JARVIS.md):\n${instructions}`);
  }
