 ...
 * Hook scripts live in .jarvis/hooks/ or are configured in .jarvis/config.json
 */
...
function getHooksDir() {
  return path.join(process.cwd(), '.jarvis', 'hooks');
}

function getConfigPath() {
  return path.join(process.cwd(), '.jarvis', 'config.json');
}

/**
 * Load hook configuration from .jarvis/config.json
 ...
 * Sources: .jarvis/hooks/{event} files + .jarvis/config.json hooks
 * @param {string} event
...
function getHooksForEvent(event) {
  ...
  // 1. Check .jarvis/hooks/{event} script
  ...
  // 2. Check .jarvis/config.json hooks
...
function runHooks(event, context = {}) {
  ...
  // Convert context to JARVIS_* env vars
  const env = {};
  for (const [key, value] of Object.entries(context)) {
    env[`JARVIS_${key.toUpperCase()}`] = String(value);
  }
